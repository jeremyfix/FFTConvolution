...
data = np.loadtxt('benchmarks_linear_everything.txt')
...
for i in range(nb_img_sizes):
    text(2+9*i,-0.07,'Image '+str(int(data[i * nb_filter_sizes+j,0]))+"x"+str(int(data[i * nb_filter_sizes+j,0])),fontsize=16)
savefig("benchmark_linear_everything.png")
savefig("benchmark_linear_everything.pdf")
show()
