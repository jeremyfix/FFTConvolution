...
data_linear_fftw = data_linear_fftw.reshape(data_linear.shape[1:]) # drop the useless mode dimension
data_circular_fftw = np.array(data[np.where(data[:,0] == 'circular'),1:], dtype=np.float)
data_circular_fftw = data_circular_fftw.reshape(data_circular.shape[1:]) # drop the useless mode dimension
...
data_linear_std = data_linear_std.reshape(data_linear.shape[1:]) # drop the useless mode dimension
data_circular_std = np.array(data[np.where(data[:,0] == 'circular'),1:], dtype=np.float)
data_circular_std = data_circular_std.reshape(data_circular.shape[1:]) # drop the useless mode dimension
...
data_linear_octave = data_linear_octave.reshape(data_linear.shape[1:]) # drop the useless mode dimension
data_circular_octave = np.array(data[np.where(data[:,0] == 'circular'),1:], dtype=np.float)
data_circular_octave = data_circular_octave.reshape(data_circular.shape[1:]) # drop the useless mode dimension
...
for i, zi in enumerate(Z_linear):
    for j, zj in enumerate(Z_linear):
        ...
        z_ratio = zi / zj
        # 
        ax = fig3.add_subplot(len(Z_linear),len(Z_linear),i*len(Z_linear) + j)
