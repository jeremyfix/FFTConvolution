...
void compute_fft_fftw3(gsl_matrix * src, gsl_matrix * dst)
{
    int height;
    int width;
    ...
    for(int i  = 0 ; i < height ; ++i)
    {
        for(int j = 0 ; j < width ; ++j)
        {
            gsl_matrix_set(dst, i, 2*j, out_src[i * width + j][0]);
            gsl_matrix_set(dst, i, 2*j+1, out_src[i * width + j][1]);
        }
    }
...
int main(int argc, char * argv[])
{
    ...
    gsl_matrix * dst_fft_fftw3 = gsl_matrix_alloc(h_src,2*w_src);
    ...
    gettimeofday(&before, NULL);
    for(i = 0 ; i < NB_REPETITIONS ; ++i)
        compute_fft_fftw3(src, dst_fft_fftw3);
    gettimeofday(&after, NULL);
    sbefore = before.tv_sec + before.tv_usec * 1E-6;
    safter =after.tv_sec + after.tv_usec * 1E-6;
    total = safter - sbefore;
    ...
    gsl_vector_memcpy(diff, &gsl_vector_view_array(dst_fft_gsl->data, dst_fft_gsl->size1*dst_fft_gsl->size2).vector);
    gsl_vector_sub(diff, &gsl_vector_view_array(dst_fft_fftw3->data, dst_fft_fftw3->size1*dst_fft_fftw3->size2).vector);
    error_value = sqrt(gsl_pow_2(gsl_blas_dnrm2(diff))/(double(diff->size)));
    printf("RMS of the FFTs : %e \n", error_value);

    // Free the memory
    gsl_matrix_free(src);
    gsl_matrix_free(dst_fft_gsl);
    gsl_matrix_free(dst_fft_fftw3);
